...
@Component({
  ...
  imports: [MatCardModule, MatButtonModule, MatFormFieldModule, MatInputModule,MatDialogClose, ReactiveFormsModule, MatSnackBarModule,MatCheckboxModule],
  templateUrl: './agregar-blog.component.html',
  styleUrl: './agregar-blog.component.css'
})
export class AgregarBlogComponent {
  private blogService = inject(HttpClient);
  form: FormGroup;
  ...
  create() {
    ...
    if (this.form.valid) {
      ...
      this.blogService.post(`${environment.API_URL}/posts`, formData).subscribe(
        (data) => {
